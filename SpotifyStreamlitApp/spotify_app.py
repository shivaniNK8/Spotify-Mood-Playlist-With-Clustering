...
#Load model and register spotify app
pipeline = joblib.load('./song-cluster-model.joblib')

# Use your spotify app credentials here
cid = 'Enter CID'
...
#Create header
st.write("""# MOODIFY!""")
st.write("""## Spotify Mood Playlists""")
...
attr_check_box = st.sidebar.checkbox("Provide audio features with mood sliders instead of artist and song name?")
...
def user_input_features():
    #create sliders if check box selected

    if attr_check_box:
        # select_slider to increase precision of slider
...
# Create playlist based on threshold and obscurity
def create_playlist(new_song, df, label, threshold = 0.1, obscurify = False):
    ...
    if obscurify == False:
        ...
        #Quantile filtering of track popularity
        popularity_quantile = playlist["track_popularity"].quantile(q)
